...
def resolve_footer_config(user_config: dict[str, Any] | None, platform_key: str | None = None) -> dict[str, Any]:
    ...
    for section in sections:
        ...
        if "style" in section:
            resolved["style"] = str(section.get("style") or "plain")
...
def format_runtime_footer(*, model: Optional[str], context_tokens: int,
                          ...
                          reasoning_effort: Optional[str] = None, style: str = "plain") -> str:
    ...
    renderers = {
        ...
        "elapsed_s": lambda: f"{elapsed_s:.1f}s" if elapsed_s is not None and elapsed_s >= 0 else "",
        "reasoning_effort": lambda: str(reasoning_effort or "").strip().lower(),
    }
    labels = {"model": "🧠", "context_pct": "🪟", "latency": "⏰", "elapsed_s": "⏰",
              "cwd": "📁", "reasoning_effort": "💭"}
    rich = style == "labeled_quote"
    parts = [f"{labels[field]} {value}" if rich else value for field in fields
             if (render := renderers.get(field)) and (value := render())]
    line = _SEP.join(parts)
    return f"> {line}" if rich and line else line
...
def build_footer_line(*, user_config: dict[str, Any] | None, platform_key: str | None,
                      ...
                      elapsed_s: Optional[float] = None, reasoning_effort: Optional[str] = None) -> str:
    ...
    return format_runtime_footer(model=model, context_tokens=context_tokens,
                                 ...
                                 elapsed_s=elapsed_s, reasoning_effort=reasoning_effort,
