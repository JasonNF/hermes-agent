...
class TurnRunner:
    ...
    def _finish_stream_consumer(self, result, agent_history, stream_consumer):
        ...
        elapsed = max(0.0, time.time() - ctx._agent_inner_started_at) if ctx._agent_inner_started_at else None
        footer = "" if is_intentional_silence_response(_final_for_stream) else build_footer_line(
            user_config=ctx.user_config, platform_key=_platform_config_key(ctx.source.platform),
            model=getattr(agent, "model", None), context_tokens=getattr(comp, "last_prompt_tokens", 0),
            context_length=getattr(comp, "context_length", None),
            elapsed_s=elapsed, turn_seconds=elapsed,
            reasoning_effort=(self._runner._reasoning_config or {}).get("effort"),
        )
        if footer:
            # Keep the authoritative finish payload and returned response equal,
            # so native streams/edit reconciliation cannot emit a second bubble.
            _final_for_stream = f"{_final_for_stream}\n\n{footer}"
            result["final_response"] = _final_for_stream
            result["footer_included_in_final_response"] = True
    ...
    def run_sync(self):
        ...
        usage = {
            ...
            "footer_included_in_final_response": result.get("footer_included_in_final_response", False),
        }
