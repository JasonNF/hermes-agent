...
class TestKernelOwnershipAndLifecycle(unittest.TestCase):
    ...
    def test_parallel_cells_share_one_kernel_process(self):
        ...
        self.assertEqual(len(_KERNELS), 1)
        # BSD pgrep (macOS) has no -c; count real direct children portably.
        import psutil
        from pathlib import Path
        live = [child for child in psutil.Process().children()
                if any(Path(arg).name == "hermes_kernel_runner.py" for arg in child.cmdline())]
